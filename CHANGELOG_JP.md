...
＃＃ 修正
- `State<TValue>.Dispatch` がディスパッチされた値が `default(T)` と等しい場合（例: `int 0`、`bool false`）にサブスクライバーに通知しない問題を修正しました。等価性チェックが状態が以前に初期化されたかどうかを考慮するようになり、値が型のデフォルトと一致しても最初のディスパッチは常に伝播されます。（Issue #4）

## [2.2.0] - 2026-02-23

＃＃ 追加した
- FluxAttribute の検出がインターフェースメソッドの定義もスキャンするようになりました。`[MethodFlux]` または `[StateFlux]` 属性がインターフェースメソッドに配置されている場合、`MonoFlux` は `GetInterfaceMap` を介して具体的な実装に自動的にバインドします。これにより、インターフェース内で直接 flux バインディングを宣言できるようになります。

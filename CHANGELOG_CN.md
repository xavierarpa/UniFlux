...
＃＃ 修复
- 修复了 `State<TValue>.Dispatch` 在分发值等于 `default(T)` 时（例如 `int 0`、`bool false`）不通知订阅者的问题。相等性检查现在考虑状态是否已初始化，因此即使值与类型的默认值匹配，第一次分发也始终会传播。（Issue #4）

## [2.2.0] - 2026-02-23

＃＃ 添加
- FluxAttribute 发现现在也会扫描接口方法定义。当 `[MethodFlux]` 或 `[StateFlux]` 属性放置在接口方法上时，`MonoFlux` 将通过 `GetInterfaceMap` 自动绑定到具体实现。这允许直接在接口中声明 flux 绑定。

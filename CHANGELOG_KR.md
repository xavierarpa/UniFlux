...
## 수정됨
- `State<TValue>.Dispatch`가 디스패치된 값이 `default(T)`와 같을 때（예: `int 0`, `bool false`）구독자에게 알리지 않던 문제를 수정했습니다. 동등성 검사가 이제 상태가 이전에 초기화되었는지 여부를 고려하므로, 값이 타입의 기본값과 일치하더라도 첫 번째 디스패치는 항상 전파됩니다.（Issue #4）
